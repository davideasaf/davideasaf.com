...
function parseImageParameters(
	src: string,
	alt?: string,
): { cleanSrc: string; params: Record<string, string> } {
...
const OptimizedMarkdownImage = ({ src, alt }: OptimizedMarkdownImageProps) => {
	...
	// Always request via Vite's asset pipeline so imagetools can transform
	// Use new URL(..., import.meta.url) which Vite rewrites to an import
	// During dev, vite-imagetools expects the /@imagetools prefix. In build, URL will be
	// resolved by Rollup. We'll construct a dev-friendly URL that still works in build.
	const makeHref = (qs: string) => `${cleanSrc}?${qs}`;

	// WebP preferred source
	const webpParams = new URLSearchParams(queryParams);
	webpParams.set("format", "webp");
	webpParams.set("imagetools", "");
	const webpHref = makeHref(webpParams.toString());

	// Fallback: original format but still optimized (resize/quality)
	const fallbackParams = new URLSearchParams(queryParams);
	fallbackParams.delete("format");
	fallbackParams.set("imagetools", "");
	const fallbackHref = makeHref(fallbackParams.toString());
